...
class Mission:
    def __init__(self, unit_id: str, team: int, target_position: Position, target_action: str = ""):  # TODO teamを追加
...
class Missions(defaultdict):
    ...
    def cleanup(self, game, player_city_tile_xy_set: Set[Tuple], opponent_city_tile_xy_set: Set[Tuple]):  # TODO player->gameに変更
        for unit_id in list(self.keys()):
            mission: Mission = self[unit_id]

            # if dead, delete from list
            if unit_id not in game.state["teamStates"][self.team].units.keys():
                del self[unit_id]
                continue

            unit = game.state["teamStates"][self.team].units[unit_id]
...
class Game:
    ...
    def init_zero_matrix(self, default_value=0):
        ...
        return np.full((self.map.height, self.map.width), default_value)

    def calculate_matrix(self, team):  # TODO teamいる?
    ...
    def calculate_resource_matrix(self, team):

        wood_fuel_rate = GAME_CONSTANTS["PARAMETERS"]["RESOURCE_TO_FUEL_RATE"]["WOOD"]
        wood_count_rate = GAME_CONSTANTS["PARAMETERS"]["WORKER_COLLECTION_RATE"]["WOOD"]
        ...
        if self.state["teamStates"][team]["researched"]["coal"]:
            coal_fuel_rate = GAME_CONSTANTS["PARAMETERS"]["RESOURCE_TO_FUEL_RATE"]["COAL"]
            coal_count_rate = GAME_CONSTANTS["PARAMETERS"]["WORKER_COLLECTION_RATE"]["COAL"]
        ...
        if self.state["teamStates"][team]["researched"]["uranium"]:
            uranium_fuel_rate = GAME_CONSTANTS["PARAMETERS"]["RESOURCE_TO_FUEL_RATE"]["URANIUM"]
            uranium_count_rate = GAME_CONSTANTS["PARAMETERS"]["WORKER_COLLECTION_RATE"]["URANIUM"]
            ...
            for x in range(self.map.width):
                if self.resource_rate_matrix[y,x] > 0:
                    for dy,dx in [(1,0),(0,1),(-1,0),(0,-1)]:
                        xx, yy = x+dx, y+dy
                        if 0 <= yy < self.map.height and 0 <= xx < self.map.width:
                            self.xy_to_resource_group_id.union((x,y), (xx,yy))

    def calculate_distance_matrix(self, distance_transition_value=12, blockade_multiplier_value=5):
        # calculate distance from resource (with fulfilled research requirements)
        visited = set()
        self.distance_from_resource = self.init_zero_matrix(self.map.height + self.map.width)
        for y in range(self.map.height):
            for x in range(self.map.width):
                if self.resource_rate_matrix[y,x] > 0:
                    visited.add((x,y))
        ...
        while queue:
            ...
            for dx,dy in [(0,1), (1,0), (0,-1), (-1,0)]:
                ...
                if (xx,yy) in visited:
                    continue
                if 0 <= xx < self.map.width and 0 <= yy < self.map.height:
        ...
        self.distance_matrix = np.full((self.map.height,self.map.width,self.map.height,self.map.width), 1001)

        for sy in range(self.map.height):
            for sx in range(self.map.width):
                ...
                while heap:
                    ...
                    for dx,dy in d4:
                        xx,yy = x+dx,y+dy
                        if not (0 <= xx < self.map.width and 0 <= yy < self.map.height):
                            continue
    ...
    def retrieve_distance(self, sx, sy, ex, ey, distance_transition_value=12, long_range_multiplier_value=5):
    ...
    def repopulate_targets(self, missions: Missions, team):  # TODO team追加したけど問題ない？
